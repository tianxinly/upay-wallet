...
async function main() {
  ...
  const now = Date.now();
  const refBlock = {
    ref_block_bytes: "3e7a",
    ref_block_hash: "8f5c2a1b9d0e3f7c",
    timestamp: now,
  ...
  const outPath = path.join(__dirname, "signed_txs.verify.json");
  fs.writeFileSync(outPath, JSON.stringify({ signed_txs: [signed] }, null, 2));
  console.log("OK: 签名成功 ->", outPath);
}
